defmodule Elastix.MappingTest do
  ...
  test "get with non existing mapping" do
    Index.create(@test_url, @test_index, %{})
    {:ok, response} = Mapping.get(@test_url, "INVALID")

    if elasticsearch_version() >= {5, 5, 0} do
      assert response.body["error"]["reason"] == "no such index [INVALID]"
    else
  ...
  test "get_all mappings should return mappings for all indexes and types" do
    ...
    assert response.status_code == 200
    assert response.body[@test_index]["mappings"] == @target_mapping
    assert response.body[@test_index2]["mappings"] == @target_mapping
  end
